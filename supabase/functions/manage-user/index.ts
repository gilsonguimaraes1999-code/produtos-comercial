import { createAdminClient, issueActivationCode, normalizeUsername, randomSecret, technicalEmailForUsername } from "../_shared/admin.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (profileId) {
      ...
      isNew = true;
      const created = await admin.auth.admin.createUser({ email: technicalEmailForUsername(username), password: randomSecret(), email_confirm: true });
      if (created.error || !created.data.user) throw created.error || new Error("AUTH_USER_CREATION_FAILED");
      authUserId = created.data.user.id;
      createdAuthUserId = authUserId;
      const inserted = await admin.from("profiles").insert({ auth_user_id: authUserId, username, display_name: displayName, role: user.role === "OWNER" ? "owner" : "commercial", status: "pending_activation" }).select("id").single();
    ...
    const activation = isNew ? await issueActivationCode(admin, profileId, actor.data.id) : null;
    createdAuthUserId = null;
    return jsonResponse({ profileId, activation });
  } catch (error) {
