alter table public.access_requests
  ...
  add column if not exists review_key uuid,
...
create unique index if not exists access_requests_review_key_idx
  on public.access_requests(review_key) where review_key is not null;

create or replace function public.submit_access_request_v2(
...
begin
  ...
  insert into public.access_requests(
    display_name, username, tracking_token_hash, submission_key
  ) values (
...
create or replace function public.get_access_request_status(
...
create or replace function public.review_access_request_v2(
  target_request_id uuid,
  decision public.access_request_status,
  approved_city_ids uuid[] default '{}',
  new_auth_user_id uuid default null,
...
begin
  ...
  if decision = 'approved' then
    if new_auth_user_id is null or coalesce(array_length(approved_city_ids, 1), 0) = 0 then
    ...
    ) then raise exception 'CITY_NOT_REQUESTED'; end if;

    insert into public.profiles(auth_user_id, username, display_name, role, status)
    values(new_auth_user_id, request_row.username, request_row.display_name, 'commercial', 'pending_activation')
  ...
  insert into public.access_history(
    access_request_id, profile_id, action, display_name_snapshot, username_snapshot,
    role_snapshot, cities_snapshot, permissions_snapshot, reviewer_id
  ) values (
    target_request_id, target_profile, decision, request_row.display_name, request_row.username::text,
    ...
    coalesce((
      ...
      from public.cities city where city.id = any(approved_city_ids)
    ), '[]'::jsonb),
    '{}'::jsonb, reviewer
  );
...
revoke all on function public.review_access_request_v2(uuid, public.access_request_status, uuid[], uuid, uuid) from public;
...
grant execute on function public.review_access_request_v2(uuid, public.access_request_status, uuid[], uuid, uuid) to authenticated;
