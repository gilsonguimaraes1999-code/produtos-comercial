...
function approveAccessRequest_(token, id, role, permissions) {
  var currentUser =
    requireOwner_(token);
...
function saveProduct_(token, input) {
  ...
  return withLock_(function () {
    ...
    if (findIndexById_(categories, normalized.categoryId) === -1) {
...
function cloneProduct_(token, productId, targetCategoryId) {
  ...
  return withLock_(function () {
    var categories = readTable_('Categories');
    if (findIndexById_(categories, targetCategoryId) === -1) {
...
function deleteProduct_(
  ...
  requireProductPermission_(token, 'deleteProduct');
...
function reorderProducts_(
  ...
  requireProductPermission_(token, 'moveProduct');
