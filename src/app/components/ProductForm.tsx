...
function findMansionStorage(product: Product | undefined, descriptionHtml: string) {
  ...
  const match = textFromHtml(descriptionHtml).match(/(?:Ba[u\u00fa]|Ba\u00fal|Storage|Armazenamento|Almacenamiento):\s*([0-9]+(?:[.,][0-9]+)?\s*(?:T|KG)?|SKORPION)/i);
  const descriptionStorage = match?.[1] ? normalizeStorage(match[1]) : '';
  return descriptionStorage || knownMansionStorageForProduct(product, [descriptionHtml]);
}
